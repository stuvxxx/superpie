...
def handle_buy(input): 
    ...
    if read.check_for_stack(new_array):
        console.print("[yellow]MESSAGE:[/] [purple]Product(s) added and we found a duplicate so stacked the products![/]")
    else:
        with open('bought.csv', 'a', encoding='UTF8', newline='') as buy_list:
            console.print("[yellow]MESSAGE:[/] [purple]Product(s) added![/]")
    ...
    if read.find_product_name(name_product):
        ...
        if quantity >= input['quantity']:
            ...
            if total_left < input['quantity']:
                console.print("[yellow]MESSAGE:[/] [purple]Damn! We dont have enough of this product to sell. Maybe try less or check the inventory for a full overview.[/]")
            else: 
                new_date_list = []
                for row in row_list:
        ...
        console.print("[yellow]MESSAGE:[/] [purple]Product not found.[/]")
...
def compare_sell_bought(row_list, order_quantity, quantity):
    ...
    for row in row_list:
        ...
        if answer[0] == 'equals':
            ...
            compare_sell_bought(row_list, order_quantity, new_quantity)
            break
        if answer[0] == "less":
...
def write_sold(bought_id, quantity, sell_price, current_date):
    ...
    with open('sold.csv', 'a', encoding='UTF8', newline='') as sell_list:
        ...
        writer.writerow(new_array)
        console.print("[yellow]MESSAGE:[/] [purple]Product(s) sold![/]")
